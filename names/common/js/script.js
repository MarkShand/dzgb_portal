...
function new_Reporter(){
   Swal.fire({
      ...
      '</div>'+
      '<div class="form-group mt-3" style="display:none;"><label class="form-label position-left">Reporter Location</label>' +
...
function send_url(){
   ...
   var ch = new BroadcastChannel('embedded_live');
   ch.postMessage(url);
}
function show_embedded_live() {
   var bc = new BroadcastChannel('embedded_live');
   bc.onmessage = function (ev) {
      received_data = ev.data.split("|");
      //document.getElementById("program").innerHTML = received_data[0];
      var url = received_data[0]
      var main_url = 'https://www.facebook.com/plugins/video.php?href=' + url + '&show_text=0&mute=0';
      $(document).ready(function(){
         $("#video").attr("src", main_url);
      });
   }
}
